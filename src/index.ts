...
export function resolveConfig(config: TokenOptimizerConfig = {}): ResolvedTokenOptimizerConfig {
  const resolved = {
...
function compactionConfigOf(config: TokenOptimizerConfig): BasicCompactionConfig {
  return {
...
function buildRetrieveTool(archive: SpillArchive) {
  return defineTool({
    ...
    async execute(args, exec) {
      ...
      if (sessionId === undefined) throw new Error('retrieve_spill requires a session owner')

      const parentSessionId = exec.agent?.session.header.parentSession
      const original = await archive.retrieve(
        String(sessionId),
...
async function mirrorToSpillStore(
  ctx: Context,
  input: SaveTextSpill,
): Promise<void> {
  const spillStore = ctx.get('spillStore') as
...
export async function apply(ctx: Context, config: TokenOptimizerConfig = {}): Promise<void> {
