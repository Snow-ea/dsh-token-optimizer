...
export async function apply(ctx, config = {}) {
    ...
    ctx.on('agent/created', ({ agent }) => {
        ...
        void archive.recordSession(String(agent.session.id), parentSessionId === undefined ? undefined : String(parentSessionId)).catch((error) => {
    ...
    ctx.on('tools/post-execute', async (exec, result, next) => {
        ...
        const parentSessionId = exec.agent?.session.header.parentSession;
        try {
            await archive.save(String(sessionId), parentSessionId === undefined ? undefined : String(parentSessionId), spillId, original);
        }
        catch (error) {
